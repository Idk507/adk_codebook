...
def delete_reminder(index : int, tool_context : ToolContext):
        ...
        ted_reminder = reminders.pop(index - 1)
        tool_context.state["reminders"] = reminders

        return {
            "action": "delete_reminder",
            "index": index,
            "ted_reminder": ted_reminder,
            "message": f"Reminder deleted: {ted_reminder}"
        }
...
memory_agent = Agent(
    ...
    description = "An agent that can remember things for you.",
    instruction="""
